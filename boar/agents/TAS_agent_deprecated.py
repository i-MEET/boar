...
class TAS_agent(Agent):
    ...
    def TAS(self,X,params,X_dimensions=[],take_log = False):
        ...
        y = []
        X_unique, X_dimensions_uni,ts = self.get_unique_X_and_xaxis_values(X,'t',X_dimensions) # get unique X values and their dimensions
    ...
    def init_args_flux_pump_models(self): 
        ...
        if self.TAS_model == Bimolecular_Trapping_equation:
            ...
            raise ValueError(f'TAS model {self.TAS_model} not implemented')
    ...
    def get_pseudo_JV(self,params, powers, Eg, Nc, T = 293):
        ...
        y = []
        X_unique, X_dimensions_uni,ts = self.get_unique_X_and_xaxis_values(Xqfls,'t',X_dimensions_qfls) # get unique X values and their dimensions
        pnames = [p.name for p in params] # get parameter names
